...
int main(int argc, char **argv) {
    ...
    TH2D* toa_starting_points_th2d = new TH2D("toa_starting_points", "ToA Starting Points;Channel;ToA Value", FPGA_CHANNEL_NUMBER_VALID*vldb_number, 0, FPGA_CHANNEL_NUMBER_VALID*vldb_number, 512, 0, 1024);
    toa_starting_points_th2d->SetStats(0);
    toa_starting_points_th2d->SetTitle("");
    
    TH2D* tot_starting_points_th2d = new TH2D("tot_starting_points", "ToT Starting Points;Channel;ToT Value", FPGA_CHANNEL_NUMBER_VALID*vldb_number, 0, FPGA_CHANNEL_NUMBER_VALID*vldb_number, 512, 0, 1024);
    tot_starting_points_th2d->SetStats(0);
    tot_starting_points_th2d->SetTitle("");
    ...
    event_adc_sum_list_copy.reserve(entry_max);
    // start event loop
        ...
        for (int vldb_id = 0; vldb_id < vldb_number; vldb_id++) {
            ...
            for (int channel = 0; channel < FPGA_CHANNEL_NUMBER; channel++) {
                ...
                for (int sample = 0; sample < machine_gun_samples; sample++) {
                    ...
                    auto &tot = val1_list_pools[vldb_id][0][channel + sample * FPGA_CHANNEL_NUMBER];
                    if (tot > 0) {
                        non_zero_tot_count++;
                        tot_value += tot;
                    }
                    auto &toa = val2_list_pools[vldb_id][0][channel + sample * FPGA_CHANNEL_NUMBER];
                    if (toa > 0) {
                        non_zero_toa_count++;
                        toa_value += toa;
                    }
                ...
                // if the non_zero_tot_count is 1, fill the starting point histogram with adc value
                if (non_zero_tot_count == 1) {
    ...
    TCanvas *toa_starting_points_th2d_canvas = new TCanvas("toa_starting_points_canvas", "toa_starting_points_canvas", 1200, 600);
    toa_starting_points_th2d->Draw("COLZ");
    toa_starting_points_th2d_canvas->SetLogz();
    TLatex latex_toa;
    latex_toa.SetTextColor(kGray+2);
    latex_toa.SetNDC();
    latex_toa.SetTextSize(0.05);
    latex_toa.SetTextFont(62);
    latex_toa.DrawLatex(0.12, 0.85, annotation_canvas_title.c_str());
    latex_toa.SetTextSize(0.035);
    latex_toa.SetTextFont(42);
    latex_toa.DrawLatex(0.12, 0.80, annotation_testbeam_title.c_str());
    // write run number, date time

    latex_toa.DrawLatex(0.12, 0.76, (std::string("Run ") + script_input_run_number).c_str());
    latex_toa.DrawLatex(0.12, 0.72, "ToA Starting Points");
    latex_toa.DrawLatex(0.12, 0.68, date_stream.str().c_str());
    toa_starting_points_th2d_canvas->Write();

    toa_starting_points_th2d_canvas->Close();

    TCanvas *tot_starting_points_th2d_canvas = new TCanvas("tot_starting_points_canvas", "tot_starting_points_canvas", 1200, 600);
    tot_starting_points_th2d->Draw("COLZ");
    tot_starting_points_th2d_canvas->SetLogz();
    TLatex latex_tot;
    latex_tot.SetTextColor(kGray+2);
    latex_tot.SetNDC();
    latex_tot.SetTextSize(0.05);
    latex_tot.SetTextFont(62);
    latex_tot.DrawLatex(0.12, 0.85, annotation_canvas_title.c_str());
    latex_tot.SetTextSize(0.035);
    latex_tot.SetTextFont(42);
    latex_tot.DrawLatex(0.12, 0.80, annotation_testbeam_title.c_str());

    // write run number, date time
    latex_tot.DrawLatex(0.12, 0.76, (std::string("Run ") + script_input_run_number).c_str());
    latex_tot.DrawLatex(0.12, 0.72, "ToT Starting Points");
    latex_tot.DrawLatex(0.12, 0.68, date_stream.str().c_str());
    tot_starting_points_th2d_canvas->Write();
    tot_starting_points_th2d_canvas->Close();
