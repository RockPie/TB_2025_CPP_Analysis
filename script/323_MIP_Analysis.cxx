...
int main(int argc, char **argv) {
    ...
    const int adc_value_bin_size = 1;
    ...
    std::vector<TH1D*> h1_adc_peak_distribution_channels(vldb_number * FPGA_CHANNEL_NUMBER, nullptr);
    for (int vldb_id = 0; vldb_id < vldb_number; vldb_id++) {
        for (int channel = 0; channel < FPGA_CHANNEL_NUMBER; channel++) {
            std::string h1_name = "h1_adc_peak_distribution_vldb" + std::to_string(vldb_id) + "_ch" + std::to_string(channel);
            h1_adc_peak_distribution_channels[vldb_id * FPGA_CHANNEL_NUMBER + channel] = new TH1D(h1_name.c_str(), h1_name.c_str(), adc_value_n_bins, adc_value_min, adc_value_max);
            h1_adc_peak_distribution_channels[vldb_id * FPGA_CHANNEL_NUMBER + channel]->SetDirectory(nullptr);
        }
    }
    ...
    for (int entry = 0; entry < entry_max; entry++) {
        ...
        for (int vldb_id = 0; vldb_id < vldb_number; vldb_id++) {
            ...
            for (int channel = 0; channel < FPGA_CHANNEL_NUMBER; channel++) {
                ...
                if (adc_peak_value_pede_sub > 0 && adc_peak_index != -1) {
                    h1_adc_peak_distribution_channels[vldb_id * FPGA_CHANNEL_NUMBER + channel]->Fill(adc_peak_value_pede_sub);
                }
            } // end of channel loop
        } // end of VLDB loop
    } // end of event loop
